...
class ChippinApi {
	...
	const API_BASE_URL_SANDBOX = 'https://sandbox.chippin.com/services';
	const API_BASE_URL = 'https://ws.chippin.com/services';
	 ...
	 * @var string
	 */
	public $debug_log_name = 'log/chippin_buynow_api.log';
	...
	public function __construct($user, $password)
	{
		...
		$this->is_debug_mode = Configuration::get('CHIPPIN_API_DEBUG_MODE');
	}

	/**
	 * Send Parameter Encryption request,
	 * return URL-encoded (by Chippin service) encryption token
	 *
	 * @param array $params
	 * @return string
	 */
	public function paramEncryption(array $params)
	{
		...
		if (!isset($xml->{'encrypted-token'}) || is_null($xml->{'encrypted-token'}))
			chippin::log('Chippin_Buynow: Invalid response data');
		else
	...
	{
		$api_url = Configuration::get('CHIPPIN_SANDBOX') ? self::API_BASE_URL_SANDBOX : self::API_BASE_URL;
	...
	public function refund($reference_number)
	{
		...
		if (!empty($response))
		{
			...
			if ($response->message && $response->message->description)
				chippin::log('API error[refund action] : '.(string)$response->message->description);
	...
	protected function request($url, $request_xml, $http_method = self::HTTP_METHOD_POST)
	{
		...
		if (!extension_loaded('curl'))
			chippin::log('cURL extension is not available');
		...
		if ($response_xml === false)
		{
			...
			chippin::log('cURL error "'.curl_errno().': '.curl_error().'"');
		}
	...
	public function getCurrencyRate($from, $to)
	{
		...
		$this->debug_log_name = 'log/chippin_exchange_api.log';
