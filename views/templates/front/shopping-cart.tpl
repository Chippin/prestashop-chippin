...
<div id="order-detail-content" class="table_block table-responsive">
    ...
    <thead>
    <tr>
        <th class="cart_product first_item">{l s='Product' mod='chippin'}</th>
        <th class="cart_description item">{l s='Description' mod='chippin'}</th>
        <th class="cart_ref item">{l s='Ref.' mod='chippin'}</th>
        <th class="cart_unit item">{l s='Unit price' mod='chippin'}</th>
        <th class="cart_quantity item">{l s='Qty' mod='chippin'}</th>
        <th class="cart_total item">{l s='Total' mod='chippin'}</th>
    ...
    {if $use_taxes}
        {if $priceDisplay}
            <tr class="cart_total_price">
                <td colspan="5">{if $display_tax_label}{l s='Total products (tax excl.)' mod='chippin'}{else}{l s='Total products' mod='chippin'}{/if}</td>
                <td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td>
            </tr>
        {else}
            <tr class="cart_total_price">
                <td colspan="5">{if $display_tax_label}{l s='Total products (tax incl.)' mod='chippin'}{else}{l s='Total products' mod='chippin'}{/if}</td>
                <td colspan="2" class="price" id="total_product">{displayPrice price=$total_products_wt}</td>
            </tr>
        {/if}
    {else}
        <tr class="cart_total_price">
            <td colspan="5">{l s='Total products' mod='chippin'}</td>
            <td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td>
        </tr>
    {/if}
    <tr{if $total_wrapping == 0} style="display: none;"{/if}>
        <td colspan="5">
            {if $use_taxes}
                {if $display_tax_label}{l s='Total gift wrapping (tax incl.):' mod='chippin'}{else}{l s='Total gift-wrapping cost:' mod='chippin'}{/if}
            {else}
                {l s='Total gift-wrapping cost:' mod='chippin'}
            {/if}
        </td>
    ...
    {if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
		{if !isset($carrier->id) || is_null($carrier->id)}
			<tr class="cart_total_delivery">
				<td colspan="5">{l s='Shipping' mod='chippin'}</td>
				<td colspan="2" class="price" id="total_shipping">{l s='Free Shipping!' mod='chippin'}</td>
			</tr>
		{/if}
    {else}
        {if $use_taxes && $total_shipping_tax_exc != $total_shipping}
            {if $priceDisplay}
                <tr class="cart_total_delivery" {if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
                    <td colspan="5">{if $display_tax_label}{l s='Total shipping (tax excl.)' mod='chippin'}{else}{l s='Total shipping' mod='chippin'}{/if}</td>
            ...
            {else}
                <tr class="cart_total_delivery"{if $total_shipping <= 0} style="display:none;"{/if}>
                    <td colspan="5">{if $display_tax_label}{l s='Total shipping (tax incl.)' mod='chippin'}{else}{l s='Total shipping' mod='chippin'}{/if}</td>
                    <td colspan="2" class="price" id="total_shipping" >{displayPrice price=$total_shipping}</td>
                </tr>
            {/if}
        {else}
            <tr class="cart_total_delivery"{if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
                <td colspan="5">{l s='Total shipping' mod='chippin'}</td>
                <td colspan="2" class="price" id="total_shipping" >{displayPrice price=$total_shipping_tax_exc}</td>
            </tr>
        {/if}
    {/if}
        ...
        <td colspan="5">
            {if $display_tax_label}
                {if $use_taxes && $priceDisplay == 0}
                    {l s='Total vouchers (tax incl.):' mod='chippin'}
                {else}
                    {l s='Total vouchers (tax excl.)' mod='chippin'}
                {/if}
            {else}
                {l s='Total vouchers' mod='chippin'}
            {/if}
        </td>
    ...
    {if $use_taxes && $show_taxes}
        <tr class="cart_total_price">
            <td colspan="5">{l s='Total (tax excl.)' mod='chippin'}</td>
            <td colspan="2" class="price" id="total_price_without_tax">{displayPrice price=$total_price_without_tax}</td>
        </tr>
        <tr class="cart_total_tax">
            <td colspan="5">{l s='Total tax' mod='chippin'}</td>
    ...
    {* Display the product line *}
        product_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}
                                ...
                                <ul class="typedText">
                                    {foreach $custom_data as $textField}
                                        <li>
                                            ...
                                            {else}
                                                {l s='Text #' mod='chippin'}{$textField@index+1|escape:'htmlall':'UTF-8'}
                                            {/if}
                                            {l s=':' mod='chippin'} {$textField.value|escape:'htmlall':'UTF-8'}
                                        </li>
                                    {/foreach}
